...
BEGIN
    ...
    END
    IF OBJECT_ID(''[?].dbo.ConnectionInformation'') IS NOT NULL
    BEGIN
        INSERT INTO #ConnectionInfo
        SELECT @LSNR AS ServerName
             , ''?'' AS DatabaseName
             , COUNT(*) AS TotalConnectionsNow
          FROM [?].dbo.ConnectionInformation
         WHERE SessionType = 1
    END
    '
