   SET NOCOUNT ON
DECLARE @dbRestoreName nvarchar(255) = 'eFinancialsAshtonProperties'
     , @deleteDate nvarchar(10) = ''
DECLARE @tempDeleteDBName nvarchar(255) = @dbRestoreName + '_TempDel' + @deleteDate
     , @dbRestoreFile nvarchar(512) = '\\' + @@SERVERNAME + '\backup\full\' + @dbRestoreName + '\eSiteCRRealEstateServices_full_20151227011054.bak'
     , @diffRestoreFile nvarchar(512) = '\\' + @@SERVERNAME + '\backup\diff\' + @dbRestoreName + '\eSiteCRRealEstateServices_diff_20151229002617.diff'  
...
DECLARE @emailbody nvarchar(2000)
   ...
   SET @emailBody = 'Restore of ' + @dbRestoreName + ' as temporary database ' + @tempDeleteDBName + ' on ' + @@SERVERNAME + ' is complete.'
                  + CHAR(10) + CHAR(13)
                  + 'Used ' + @dbRestoreFile + ' and ' + @diffRestoreFile + ' to create ' + @tempDeleteDBName
