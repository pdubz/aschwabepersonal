...
ToDo: error handle better -- if no AG, if db isn't in AG, if db is not on one of the nodes
      sprocify and add flags for usage, final backup to disk, print commands
*/

declare @dbDeleteName nvarchar(128) = N'CUNY_PLT_Objects'
               ...
               begin
                    --backup database
                      print 'Backing up database on ' + @@SERVERNAME
                       exec dba.dbo.usp_backup_db @bu_type = 'full', @dbname = @dbDeleteName, @comment = 'FINAL'
