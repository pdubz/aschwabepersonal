...
BEGIN
    ...
    END
    IF OBJECT_ID(''[?].CORE_ACCESSCONTROL.ACCESSUSERACTIVITY'') IS NOT NULL
    BEGIN
        INSERT INTO #UserCountInfo
        SELECT @LSNR AS ServerName
		     , ''?'' AS DatabaseName
		     , COUNT(DISTINCT USERKEY) AS ActiveUsers
          FROM [?].CORE_ACCESSCONTROL.ACCESSUSERACTIVITY 
         WHERE APPLICATIONNAME = ''IPS''
    END
    '
    
      EXEC dba.dbo.usp_foreachdb @command = @UserCountQuery
                               , @suppress_quotename = 1
                               , @name_pattern = '%PRD_IPS_IPS_%'
