...
if not exists(
    select 1
     ...
     where ss.name = 'dbo'
       and so.name = 'SendToSupport'
)
begin
    exec('create procedure dbo.SendToSupport as begin set nocount on end')
end
go

ALTER PROCEDURE [dbo].[SendToSupport]    
...
BEGIN
    ...
    IF NOT EXISTS ( SELECT *
        ...
        BEGIN
            ...
            IF @testBackup = 'Y'
            BEGIN
                SET @cmd = 'powershell.exe -Command "& C:\salt\scripts\SendToSupport\utilTest.ps1 -utilServerName '+ @utilServerName +' -primaryServerName '+ @@SERVERNAME +' -logID '+ CONVERT(nvarchar(10),@logTableID) +' -tmpDBName '+ @tempDBName +'"'
                print @cmd
            ...
            BEGIN
                --call zip to upload
                SET @cmd = 'powershell.exe -Command "& C:\salt\scripts\SendToSupport\ZipUpload4Support.ps1 -fullBakFilePath ' + @afterDecryptBackupFilePath + ' -logTableID ' + CONVERT(nvarchar(10),@logTableID) + ' -code ' + @code + ' -primaryServerName '+ @@SERVERNAME +'"'
            ...
            DECLARE @dataFilePath    nvarchar(1000) = 'E:\data01\data\' + @tempDBName + '_data.mdf'
                ...
                ,    @utilDFilePath    nvarchar(1000) = '\\' + @utilServerName + '\Data\data\' + @utilDBName + '_data.mdf'
                ,    @utilLFilePath    nvarchar(1000) = '\\' + @utilServerName + '\Logs\data\' + @utilDBName + '_log.ldf'

            --Begin shredding files
            SET @cmd = 'powershell.exe -Command "& C:\salt\scripts\SendToSupport\shredFiles.ps1 -fullFilePaths "' + @dataFilePath + '", "'+ @logFilePath +'", "'+ @afterDecryptBackupFilePath +'"'
            IF @testBackup = 'Y'
            BEGIN
            ...
            BEGIN TRY
                ...
                EXEC dba.dbo.sp_query_to_html_table @html = @table OUTPUT,  @query = @logQuery
